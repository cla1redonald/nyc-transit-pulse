...
import { useFilters, type DatePreset } from '@/lib/filter-context'
import { DATE_RANGE_PRESETS } from '@/lib/constants'
import { MODE_COLORS, MODE_LABELS_SHORT } from '@/lib/colors'
  ...
  return (
    <div className="sticky top-14 z-40 bg-background/95 backdrop-blur-sm supports-[backdrop-filter]:bg-background/60 border-b">
      <div className="py-4 space-y-3">
        ...
        {/* Mode Filter Chips */}
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
          <span className="text-sm font-medium text-muted-foreground whitespace-nowrap">Modes:</span>
          <div className="flex gap-2">
            {MODE_KEYS.map((mode) => {
