...
import type { DayOfWeekData, TransitMode } from '@/types/transit'

// Import data
import dowData from '@/data/dow.json'
...
export function DayOfWeekChart() {
  ...
  const chartData = useMemo(() => {
    ...
    if (viewMode === 'total') {
      ...
      return data.map((item) => ({
        ...
        ...(activeModes.has('metroNorth') && { metroNorth: item.avgMetroNorth }),
        ...(activeModes.has('accessARide') && { accessARide: item.avgAccessARide }),
        ...(activeModes.has('bridgesTunnels') && { bridgesTunnels: item.avgBridgesTunnels }),
  ...
  const peakDay = useMemo(() => {
    ...
    const sorted = [...chartData].sort((a, b) => {
      ...
      if (viewMode === 'total' && 'total' in a) {
        ...
        // Sum only numeric values, excluding dayOfWeek string
  ...
  const maxTotal = useMemo(() => {
    ...
    return Math.max(...chartData.map((d) => ('total' in d && typeof d.total === 'number' ? d.total : 0)))
  }, [chartData, viewMode])
          ...
          <Tooltip
            content={({ active, payload, label }) => {
              ...
              if (viewMode === 'total') {
                ...
                return (
                  <div
                    className="rounded-lg border border-border bg-surface-raised p-3 shadow-lg"
                    style={{
                    ...
                    <div className="space-y-1">
                      {activeModes.has('subway') && (
                        <div className="flex items-center gap-2">
                          <div
                            ...
                            style={{ backgroundColor: MODE_COLORS.subway }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.subway}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.subway)}
                          </span>
                        </div>
                      )}
                      {activeModes.has('bus') && (
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2 w-2 rounded-full"
                            style={{ backgroundColor: MODE_COLORS.bus }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.bus}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.bus)}
                          </span>
                        </div>
                      )}
                      {activeModes.has('lirr') && (
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2 w-2 rounded-full"
                            style={{ backgroundColor: MODE_COLORS.lirr }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.lirr}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.lirr)}
                          </span>
                        </div>
                      )}
                      {activeModes.has('metroNorth') && (
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2 w-2 rounded-full"
                            style={{ backgroundColor: MODE_COLORS.metroNorth }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.metroNorth}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.metroNorth)}
                          </span>
                        </div>
                      )}
                      {activeModes.has('accessARide') && (
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2 w-2 rounded-full"
                            style={{ backgroundColor: MODE_COLORS.accessARide }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.accessARide}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.accessARide)}
                          </span>
                        </div>
                      )}
                      {activeModes.has('bridgesTunnels') && (
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2 w-2 rounded-full"
                            style={{ backgroundColor: MODE_COLORS.bridgesTunnels }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.bridgesTunnels}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.bridgesTunnels)}
                          </span>
                        </div>
                      )}
                      {activeModes.has('sir') && (
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2 w-2 rounded-full"
                            style={{ backgroundColor: MODE_COLORS.sir }}
                          />
                          <span className="text-xs text-muted-foreground">
                            {MODE_LABELS.sir}:
                          </span>
                          <span className="text-xs font-medium text-foreground">
                            {formatCompactNumber(data.sir)}
                          </span>
                        </div>
                      )}
                    </div>
            ...
            <Bar dataKey="total" name="Total Ridership" radius={[4, 4, 0, 0]}>
              {chartData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={getBarColor(
                    'total' in entry && typeof entry.total === 'number' ? entry.total : 0,
                    maxTotal
                  )}
                />
              ))}
      ...
      {peakDay && (
        <div className="mt-4 rounded-lg bg-muted/50 p-3">
          <p className="text-xs text-muted-foreground">
            <span className="font-medium text-foreground">{peakDay}</span> sees the highest
            ridership on average, reflecting peak midweek commute patterns.
          </p>
        </div>
      )}
    </div>
